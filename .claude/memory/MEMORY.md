...
- [Cross-platform scripts (macOS + Windows)](feedback_cross_platform_scripts.md) — claude/scripts run on both OSes; actually run the Windows branch, don't leave a print-only stub
- [Terminal width detection](reference_terminal_width_detection.md) — real terminal width only via PowerShell tool's `$Host.UI.RawUI.WindowSize.Width`; subtract ~2 for the TUI gutter
