...
## A second argument, from the one version that tried a script anyway

v10 re-checks v1's routing, and it was written as an `apply.py` on the reasoning authoring.md allows — the work looked mechanical and tedious. It was thrown away on 2026-09-18 after four adversarial rounds confirmed 31, 35, 53 and 46 defects across three successive designs of it. Each design was blind to something the previous one caught: joining a memo to its checklist line on the capture stamp mistook an unrelated same-minute capture for a lost memo's file, and counting markers against directories instead reported two memos swapped across the two sides as correct, because the counts cancel.

What that produced is now the transformation-versus-judgement test in `claude/conventions/authoring.md`, beside the sentence permitting an `apply.py`; it is written there rather than here because that is the file someone reads while deciding. What this memory keeps is the part specific to this backlog: every key available for joining a checklist line to its memo was lossy, and all three at once. The stamp is a prefix of `created:` rather than equal to it, since `memos.py` stores seconds the checklist never held; it is not unique, because two memos captured in one minute share it; and v1's own "Afterwards" step invites rewriting the title the slug was derived from. An agent reading the two lists side by side settles in seconds what none of those keys can, and 92 items across ten repos is a list rather than a dataset.
