...
- **Anchor resolution can fail for a top-layer element that is a descendant of its own anchor.** The arrangement that works fine in normal flow — tip nested inside the hoverable, `anchor-scope` on the hoverable — can stop resolving once the tip is promoted to the top layer (`showPopover()`), particularly inside a transformed subtree. When `anchor()` fails it is invalid at computed-value time, the insets collapse, and the element lands against a viewport edge (a tooltip pinned to the far left is the tell). The top layer already gives the viewport as containing block, so `getBoundingClientRect()` plus explicit `top`/`left` is the reliable fallback — and set `position-anchor: none; position-try-fallbacks: none` on that variant so the CSS can't fight the inline coordinates. Keep CSS anchoring for the non-top-layer surface; only the promoted one needs JS.
- **A `:hover` cancel rule silently out-specifies `:popover-open`.** Converting a hover tooltip to a popover invites `.anchor:hover > .tip { display: none }` to "cancel" the old reveal — but that is (0,3,0) against `.tip:popover-open { display: block }` at (0,2,0), so the tip is forced hidden at exactly the moment it is open. Delete the cancel rather than adding it; and check whether the hover rule you are cancelling is even in the injected/loaded sheet.
- **The popover UA sheet fights anchor geometry.** It sets `inset: 0`, `width/height: fit-content`, `margin: auto`, plus border/padding/background. Author rules already beat it, so reset only the *decorative* properties — restating `inset`, `width` or `height` in your own `[popover]` rule overrides your anchor-based `top`/`left` and box. Use `background-color`, never the `background` shorthand, if the element carries a `background-image`.
