...
Prototyping this live (see the note further down on iterating in an authenticated session) has one trap that hides the whole problem: a probe stylesheet with `!important` on every declaration proves only that the *selectors* match, never that the shipped rule wins. Everything works in the probe and the same CSS does nothing once injected, which reads as a build or reload problem rather than a cascade one. Probe with exactly the declarations you intend to ship, and add `!important` only where a measured inline style demands it — `Array.from(el.style)` lists the properties the host has claimed, and is the cheapest way to decide before writing the rule rather than after the user reports it.
...
**Resizing a host's sprite box re-crops the sprite — scale it with `zoom` instead.** A sprite tile is only correct at the size its author picked, and how it fails when you change that size depends on a detail you cannot see from the markup: whether `background-position` is in percentages or pixels. Percentages are resolved against `(box − image)`, so a tile positioned that way re-fits a smaller box on its own and survives. Pixels do not scale, so against a percentage `background-size` the sheet shrinks underneath a fixed offset and you get a *different tile* — not a distorted one, a wrong one, which reads as a data bug rather than a CSS one. One host can use both conventions in the same row: five counter icons where four were percentage-positioned and the fifth (from a different sheet) was `-100px -262px` failed only in the fifth.

`zoom` on the counter's wrapper is the fix, and is better than it first looks: it leaves every value the host authored untouched — box, offset, sheet size — and scales the rendered result, so a sprite aligned at full size stays aligned at any factor. It also takes the text and the layout box down with it, so a row of icon-plus-number scales as one unit and is laid out at what is drawn. `transform: scale()` shares the sprite-safety but not the layout part (it leaves the box at its old size), so reach for it only when you want the box left alone — and remember it overwrites any `transform` the host was already using (see above).

Two corollaries. Host tiles usually fill their cell edge to edge, so any icon you draw yourself needs to fill its viewBox or it will sit visibly smaller beside them — worth measuring the sheet's alpha bounds rather than guessing. And a mis-cropped tile is invisible to every check that does not look at pixels: the element is the right size, the image loads, and the computed `background-position` is exactly what the host set.
...
If your own restyle turns a host element into a scrollport — pinning a column and capping it with `max-height` + `overflow-y: auto` — that element gets a second, independent bounce, and anything stuck *inside* it is dragged by that one instead. The root rule cannot reach it; it needs its own. Use `none`, not `contain`: `contain` stops the scroll chaining and leaves the bounce, and only `none` suppresses both. Losing the chain costs nothing when the column is capped to the window anyway. Note the two suppressions have different conditions and may live in different injected sheets — a header's rule scoped to "the bar is folded" does not cover panels that stay pinned when a long prompt unfolds it.
...
**The API reports the published version, so nothing in the repo needs to record it.** `GET https://www.googleapis.com/chromewebstore/v1.1/items/<id>?projection=DRAFT` returns only `kind`, `id`, `publicKey`, `uploadState` and `crxVersion`. `crxVersion` is the version the store holds; `uploadState` says which kind it is — `NOT_FOUND` means no draft is pending, so `crxVersion` is the published package, while any other value means it's an uploaded-but-unsubmitted draft and the live version is older. That pair answers "which version is published" better than a stored stamp can, because a stamp is written at *submission* and review can still reject it days later.

**Everything on the Privacy practices and Store listing tabs is dashboard-only.** Single purpose, per-permission justifications, store description, data usage and the privacy policy URL can be neither read nor written through the API — it does package upload and publish, nothing else. So a repo-side mirror of that text is an unverifiable cache: edit the dashboard directly and nothing will ever notice the copy went stale. Derive the pre-flight check instead. Against the store's `crxVersion`, `git diff v<crxVersion>..HEAD -- manifest.json` surfaces all three things that block a submission — a permission added (the dashboard has no justification for it and will reject), a permission removed (its justification is now orphaned), and a changed `description` (which the store Description and the Single purpose statement usually mirror).
