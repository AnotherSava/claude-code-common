...
Never find the caller's repo from a path argument such as an output file: a caller may stage that file in `%TEMP%`, outside every repo, and a lookup that throws there can lose the only copy if the caller deletes the file in `finally`.
