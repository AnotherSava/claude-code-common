...
- **That PUT will happily commit an empty file, and the job stays green when it does.** Building the payload inline as `-f content="$(base64 -w0 "$CASK")"` breaks the moment the job runs on `macos-latest` (which you want, for `brew audit`): BSD `base64` has no `-w`, the substitution fails, and `set -euo pipefail` does not catch a failure inside an argument word, so the API is handed `content=""` and does as it is told. Measured cost on this exact setup: a 0-byte cask shipped for two releases with four green runs and no notification, because the validate step was gated on a `git diff` that an empty rewrite also satisfies, and a `GITHUB_TOKEN` push cannot trigger the tap's own test workflow. Use `base64 -i "$CASK" | tr -d '\n'` (or `openssl base64 -A`), and after the PUT read the file back and assert it is non-empty and carries the expected `version`. Full anatomy in `github-actions-silent-write-failures.md`.
- Verify a bump by installing it, not by reading the run. `gh api repos/OWNER/REPO/contents/Casks/<name>.rb --jq '{size,sha}'` returning `size: 0` / `sha: e69de29…` (git's empty blob) is conclusive proof the cask is broken, and it is the only cheap check that a green pipeline cannot fake.
