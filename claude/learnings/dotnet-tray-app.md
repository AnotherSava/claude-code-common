...
**Measurement before show**: `window.Measure()` before the window is in the visual tree returns `DesiredSize` **(0,0)** — width *and* height, not just height (a `desiredW=0` in logs is the tell). Three ways to get a real size for pre-Show placement: (a) measure the **content element** (e.g. the root `Border`) instead of the `Window` — its `DesiredSize` *does* include its own `LayoutTransform` scale and is reliable before Show; (b) if a dimension is fixed/deterministic (all content fixed-width), just compute it (`baseWidth × scale`) and skip the measure; (c) `Dispatcher.BeginInvoke` at `DispatcherPriority.Loaded` after `Show()` to read `ActualHeight`/`ActualWidth` and correct position. **Smell:** two placement paths that each fall back differently when the discarded measure is 0 will silently diverge (one applied the scale to its fallback, the copy-pasted sibling didn't → different on-screen position) — share one placement helper instead.
