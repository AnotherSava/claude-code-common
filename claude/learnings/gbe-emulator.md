...
The quoting is **not** universal, so a parser must accept both spellings rather than picking one: surveyed across 11 real `steam_settings` schemas on one machine (2026-09-03), `hidden` is a quoted string in 8 and a bare JSON number in 3. Every one of the 11 carries the field. In the same survey the blank-`description` count is always **either 0 or exactly the hidden count**, never in between — so "some descriptions are empty" is never a partial failure, it is either Steam's redaction showing through untouched or a generator that filled them all in. Atomfall is the decisive case: its two `steam_settings` folders declare the same 25 hidden achievements, and the `bin/coldclient/` copy has 25 blank descriptions where the root copy has 0.
...
### Localisation stops at the hidden set, and no source fills the gap
