...
## A negation under an excluded directory is dead

Write `docs/plans/*`, not `docs/plans/`, when exceptions follow. Git can't re-include anything under a directory it has excluded, so after `docs/plans/` the lines `!docs/plans/completed/` and `!docs/plans/draft/` match nothing and change nothing. The failure is quiet: a file already tracked there stays tracked, so the exception looks like it works, and only a *new* file placed there is ignored. `git check-ignore -v <new file>` shows it, naming the `docs/plans/` line. Found 2026-09-24 in url-cleaner, where a finished plan moved into `docs/plans/completed/` to be published would have stayed out of the commit. `docs/plans/*` excludes the directory's entries instead of the directory, so the exceptions apply and every other file there stays ignored.
