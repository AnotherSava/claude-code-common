...
**Migrating into the global convention (2026-08-22, printlab `publish`):** where the global skill is already general enough and the project simply predates it, the fix is neither forking nor generalizing — migrate the project into the global's config shape (`config/<verb>.env` + `scripts/<verb>.sh`) and keep a project-local skill under a **non-colliding** name for the genuinely project-specific remainder (prod DB changes, secret refresh, DNS, rollback). Note the detection gap that hides this case: a global script-based skill probes only for *its own* artifacts, so a project configured any other way reports "not configured" and drops into first-time setup against a deployment that has been working for months. And if the generic script lacks something the project's own path had — printlab's serve-check asserted identity, the generic one only liveness — fix the generic script first, or the migration is a silent downgrade.
