...
Some work needs the computer itself and not just a shell: raising a window to the foreground, moving the pointer, synthesizing a keystroke or a click, dragging or resizing a window, driving a GUI. For as long as that runs the machine is not the user's — a click lands wherever the pointer was put, a keystroke goes to whatever was raised, and anything they were typing goes somewhere they did not choose.

**Ask immediately before starting, every time.** Not at the top of the task, and not folded into a plan approved twenty minutes ago: the confirmation has to sit right against the interaction, so the user knows the next stretch of seconds belongs to the agent and can decline if they are mid-sentence in another window. Say what will be driven and roughly how long it takes. Approval covers that burst and no other — a second burst needs a second ask.
...
**When research has to come first, research first and ask afterwards.** A confirmation given before the investigation is a confirmation for work nobody had scoped yet — the answer would be to something vaguer than what actually gets driven. Read, probe and measure with ordinary tools, work out exactly which windows get touched and for how long, and ask then, once the question is specific.

**Say so on the dashboard's tray icon, so the state is visible without reading chat.** Three states, each distinguishable at tray size: the normal icon when nothing is happening; a distinct variant while a request is pending, so a user away from the transcript can see that an agent is waiting on them; and a distinct, animated variant while the interaction is actually running, so a machine moving on its own is never ambiguous. It returns to the normal icon the moment the burst ends, including when the burst fails or is declined — an icon stuck on "interacting" is worse than no icon, because it teaches the user to ignore it.
...
**The same check applies before *writing*, not only before diagnosing.** A learning named after the format, API or tool you are about to code against is a spec someone has already paid for. Real case 2026-09-12: `git-porcelain-parsing.md` opens with the `.strip()` trap and closes with the `-z` flag, and a script parsing `git status --porcelain` was written without reading it — reproducing that exact `line[3:]` off-by-one, and shipping the non-ASCII bug the same file warns about. Nothing in the task looked like diagnosis, which is why the rule above never fired. Glob the directory for the nouns in the task before the first line of code, not after the first failing test.

When a problem resists the first attempt or two — especially browser/CSS quirks, framework behavior, tool errors, or library/API limitations — search the web for the root cause and known fixes instead of iterating blindly or concluding it can't be done. A targeted search often surfaces a proven solution, and explains *why* the naive attempts failed, faster than guess-and-check. Lean toward researching early when the territory is unfamiliar or a fix isn't converging; reserve trial-and-error for cases where simply trying it is genuinely cheaper than a search. The repeated-experiment smell (three tweaks, three failures) is the signal to stop and look it up. Real case 2026-09-24: about eight rounds of pixel heuristics on Windows screenshot borders (lift, recolour, copy another frame's edge), each shipping a new defect, until the user asked for Windows' own algorithm; one research pass (docs, the uDWM disassembly, measurement of the captures) produced a model that reproduced them to 0.2 levels RMS. Reproducing how a platform renders something is modelling, not tuning: find the algorithm first.
...
- When a task incidentally changes a file in a *different* repository than the one the task is about (e.g. editing a global skill in the dotfiles repo while working in a project), report the change so the user knows it's there, and never commit it yourself — that repo's commits are not this session's to drive. Don't press the user to go and commit it either; surface, don't solicit. The one route onward is `/commit` step 10, which asks the live session that owns that repo to commit it under its own review. That is routing rather than soliciting: the ask reaches that repo's user through their own session, which is where the decision belongs, and it is scoped to repos the session actually wrote to — never to a repo merely found dirty.
...
- **Check the remote before starting a new request that follows an idle gap.** When a request arrives roughly 30+ minutes after the previous one — the session transcript's last two timestamps give the gap — run `git fetch -q && git log --oneline HEAD..@{upstream}` before touching anything; a "no upstream configured" error just means there is nothing to sync. These repos are worked on from both a Windows and a macOS machine, so an idle gap is exactly when the remote moves ahead, and a stale tree means rebuilding what already exists upstream, or a `/commit` that cannot push. When the remote is ahead, read the incoming commit messages before diagnosing anything — they often explain the environment you are about to fight (a dev server pinned to one port, a publish workflow) — and propose a rebase rather than running one. The gap can be days, and a long-lived session is where it hides — the trigger is any turn after a pause, a skill invocation included, not only a typed request. Re-verify anything decided before the gap rather than acting on the old approval: the action may already have been done elsewhere, and an untracked derived artifact (a generated directory, a migrated file) may have been built from the pre-gap tree, so check it against the remote's source rather than against what you started with.
...
A convention written here is a claim about every repo on this machine, so changing one leaves the rest in the old shape: the one such change made by hand so far missed a repo entirely and left five more holding the work uncommitted. So a change that an already-conforming repo must **do something** about ships as a numbered migration — one folder under `claude/conventions/versions/`, its number in its name, carrying prose an agent follows and the evidence that settles when it does not apply — committed alongside the change that caused it. Each repo records one integer in a committed `.claude/conventions`, how far that sequence has run there; a SessionStart hook prints the gap once per session, and `/adopt`, run in the repo that is behind by the session that owns it, walks the pending versions one at a time and hands the number to `/commit`. What must hold *continuously* is never a version but a **rule** under `claude/conventions/rules/`, run by `claude/conventions/check.py` from that repo's commit gate — every rule its number entitles it to, at every commit, with a rule that could not look reported as unmeasured rather than as a pass. Beside those sits the smaller class in `claude/conventions/universal/`, gated by no number and so running in every repo whose gate calls the checker, with no adoption in between: what belongs there is a property no migration could settle for good because it is not about the repo alone and whose violation nothing else would report, and each one names the command that repairs what it found. Anything a repo can adopt stays a version. What can never carry a version at all is enumerated in `claude/conventions/not-versioned.md`, so "current" means every versionable convention has been decided rather than every rule in this file being satisfied.
...
- **Routing.** Reporting is always required. Fixing unasked is limited to the change you are already making; anything wider becomes an offer to the user, a `.claude/memos/` entry, or its own commit, because an unrelated fix folded into a scoped diff makes the commit message describe a change it does not contain. The boundary is the code you are editing, not the feature you are delivering: a defect in the same function, and especially in the same block or statement you just modified, is inside the surface by definition and gets fixed now. Real case: a too-wide `try` was reported and left alone as "wider work" while its own `catch` was being rewritten in the same edit. A small, concrete defect in a file the current investigation is *about* is inside the surface too — fix it and report it, rather than offering: an investigation that edits nothing still leaves you holding the file, and the offer costs a round trip to approve what was always going to be approved. Narrow-width layout problems stay governed by `~/.claude/memory/feedback_desktop_first_then_phone.md`: note them and move on while the desktop look is unsettled.
...
- **Never take a generic name in a namespace someone else shares.** Compose publishes a *service's* name as a DNS alias on every network it joins, so `app`, `web`, `db`, `caddy` and friends are claims staked on a shared bridge, not local labels. The same applies to any flat shared namespace: a vhost dropped into a common `conf.d` is separated from its neighbours only by its basename, so `site.caddy` overwrites theirs. **A DNS zone you own is one of these namespaces too** — every project taking hostnames in `example.com` shares it, and a Workers custom domain binds a hostname to exactly one Worker, so the first project to claim `notify.example.com` takes it from all of them. Owning the zone is what makes this instance easy to miss. Name the thing after its project (`scheduler-app`, `scheduler.caddy`, `scheduler-notify.example.com`), make the service name equal the container name, and dial upstreams by container name, never by service name.
...
- **Report-announcing lead-ins** — "Two things I'd rather say than have you find", "Before you ask", "For completeness", "It's worth flagging that", "One thing to note", "I should mention". They announce the act of reporting instead of reporting, and they sit anywhere in a reply rather than only at the front, which is why the plan-announcing entry above does not catch them. Delete the lead-in and state the thing: "The dashboard shows no rows until then", not "Two things I'd rather say than have you find: the dashboard shows no rows until then". The same reflex one step further announces your own candour or the importance of what follows — "Worth stating plainly", "So:", "One expectation:", "Heads up either way", "One limit worth knowing about". Being candid is done by saying the thing; a sentence claiming you are about to be candid is the opposite. Drafting a GitHub reply, the user read one back and asked "who'd writing like this" — it reads as performed, not plain.
...
Describe the thing as it is, not the drafts it went through — **and this one binds a code comment of any length**, which is where it is broken, since a comment is written in the same breath as the change and the alternative that was just abandoned still feels like part of the explanation. "Putting the provenance first pushed the date off its column", "this used to be a flag", "the earlier version recomputed it" all describe code no reader can see, and each ages into a claim about something that no longer exists. Keep it in one case only: the discarded shape is one a reader could plausibly reach for again AND choosing it costs something real, in which case say what it costs rather than that it happened — "listing only the wildcard leaves `/inbox` reachable from the internet, which it was for a day". The test is whether somebody who never saw the old version is at risk of writing it; an iteration from the last hour almost never is, and the fix for one is to delete the sentence rather than to rephrase it.
...
- **The same tool pointed at a different config is differently *scoped*, not weaker** — and it reports success just as confidently. `npx svelte-check` resolves the *root* tsconfig; CI ran `npm run check`, which passes `--tsconfig ./tsconfig.app.json`, and failed on a file outside the first one's scope. So read the project's own `check`/`lint`/`test` scripts in `package.json` (or the CI workflow) and run *those*, rather than a command that resembles them.
...
**A grep that enumerates the duplicates is the extraction trigger, not a worklist.** Having found every call site, the reflex is to apply the same change to each — which ships the duplication intact and doubles what the next fix has to find. Real case 2026-09-19: one grep found a broken width detector in two skill helpers and three documents; the fix went into them one by one until the user asked why it was not extracted, and one shared module then replaced all of it.
...
- **Global excludes** (`git config --global core.excludesfile` — on this machine `~/.gitignore`, not `~/.gitignore_global`): OS- or user-specific files no contributor shares — IDE folders, OS caches, tool outputs (`.idea/`, `Thumbs.db`, `.ralphex/`), and per-machine wrappers written by personal skills (the deploy/build/cleanup/publish/doppler skills' `scripts/*.sh` wrappers and the per-machine `config/deploy.env` they read). Note `config/publish.env` is deliberately **not** among them any more: it is versioned, encrypted with transcrypt, because an ignore rule cannot stop per-machine copies drifting apart — see `git/gitignore` for the current set and the reasoning. Only narrow **root-anchored file** patterns belong here — never a floating whole-dir pattern like `scripts/`, which would hide that dir in *every* repo, including ones that commit a real `scripts/`. (A file pattern with a mid-string slash like `scripts/deploy.sh` is already root-anchored.)
...
Cross-project preferences and feedback. Memory files live in `~/.claude/memory/`. When saving a memory that applies across all projects (not just the current one), write the file there and add its line to `~/.claude/memory/MEMORY.md`. Same frontmatter format as project-specific memories.

**That file is the only index anyone edits, and it lists every memory exactly once.** The block below is generated from it by `claude/scripts/render-memory-index.py` and must not be hand-edited — a commit check re-renders it and fails on any difference. What the block contains is the subset marked `{always}` there: the memories that have to fire unprompted, because this file is injected into every session and `MEMORY.md` is not read at all unless something goes looking. So marking an entry `{always}` buys presence at the cost of context, and leaving it unmarked still leaves it findable by anything that reads the index.
...
<!-- BEGIN generated by claude/scripts/render-memory-index.py — edit claude/memory/MEMORY.md, never this block -->
...
- [Prefer loud errors to silent fallback](~/.claude/memory/feedback_loud_errors.md) — surface failures in UI + log; never silently degrade; show status on the active view, prefer truthful "retrying" over a timeout that fakes a terminal result
...
- [Check the source before deriving](~/.claude/memory/feedback_check_the_source_before_deriving.md) — before writing a computed value over stored data, check whether the system that imported it already holds the right one
- [Overused phrases](~/.claude/memory/feedback_overused_phrases.md) — live blocklist of verbal tics; the list itself is CLAUDE.md's **Overused Phrases** section, and grows as new ones surface
- [Never blind-splice a file region](~/.claude/memory/feedback_no_blind_splice.md) — read what lies between the anchors first; marker/index splices silently delete unrelated code with no error at edit time
...
- [Guard on the version, not the artifact](~/.claude/memory/feedback_guard_on_version_not_artifact.md) — a tool reading a format defined elsewhere checks the adopted version, not whether the old artifact is still there; sniffing it rots at the next migration
- [Lead with the rule, not the value](~/.claude/memory/feedback_lead_with_the_rule.md) — a request naming a value, answered with a rule: say the rule in the FIRST sentence or the number reads as hardcoded
...
- [Mark the container, not every item](~/.claude/memory/feedback_mark_the_container_not_every_item.md) — a scope marker says where the work must happen, not that every item inside is bound; don't withhold it because one part is portable
...
- [Always report images with a contact sheet](~/.claude/memory/feedback_image_report_always.md) — reporting anything about screenshots means building the HTML sheet and linking it; prose and transcript images are not substitutes
...
- [Captured the lesson, drop the code](~/.claude/memory/feedback_research_to_production_cleanup.md) — when research code transitions to production, delete helpers whose rationale lives in docs; the memory that replaces a retired tool keeps the claim, not the apparatus
...
- [Make the state settable, not a viewer](~/.claude/memory/feedback_make_the_state_settable.md) — expose the input a view depends on; never build a scaffold that imitates the state
...
- [Use Doppler for secrets](~/.claude/memory/feedback_doppler_secrets.md) — **invoke `/doppler` before planning where a secret lives or writing any `doppler` command.** All 10 project slots are taken: a new app gets a config in an existing shard (`prd_<app>`), never its own project. Command-time landmines (quoting, `--silent`, root-vs-branch) are injected by the doppler-guard hook when they apply.
...
- [Check a destination is not published](~/.claude/memory/feedback_check_destination_visibility.md) — check `gh repo view --json isPrivate` AND `git check-ignore` before moving anything to a shared repo; then redact live-data figures from what goes in
- [Rotate, don't abandon](~/.claude/memory/feedback_rotate_dont_abandon.md) — a leaked credential makes one value worthless, not the access; propose replacing it, say what survives, verify the old one is dead
...
- [Sampling a level misses an edge](~/.claude/memory/feedback_sample_level_miss_edge.md) — polling "is it in state X" can't catch "did X happen"; a shorter interval narrows the blind window and never closes it — get an event, or state the limit
- [Startup is not a poll](~/.claude/memory/feedback_startup_is_not_a_poll.md) — a startup-only concern gets bounded retry-until-*answered*, not a timer; count the recurrence before defending it, and check what the "cheap" gate calls
- [Check where it is consumed](~/.claude/memory/feedback_check_where_it_is_consumed.md) — a session-start assertion is blind for the whole session; put a precondition check on the action that needs it
- [Reversible edit, not backup-and-restore](~/.claude/memory/feedback_reversible_over_backup.md) — temp file mutations for a check: self-inverse sed; backups collide on basename and zsh `$var` in a for-list silently no-ops
- [A probe command still writes](~/.claude/memory/feedback_probe_command_still_writes.md) — re-running something to read its output performs every write it does; empty input made one re-render blank a full cache
...
- [Check the limit is real](~/.claude/memory/feedback_check_the_limit_is_real.md) — check it isn't already handled: evidence the system holds, a general rule whose premise fails here, work a later step owns, or a sibling project already built
...
- [A marker must be distinct](~/.claude/memory/feedback_marker_must_be_distinct.md) — a mark means something only by being unlike the others; reusing the nav marker for a notice made both meaningless
- [Warnings lead with the instruction](~/.claude/memory/feedback_warning_leads_with_instruction.md) — what to do first, why second, and the reason must be one the reader couldn't supply themselves
- [Fix the part, not the whole](~/.claude/memory/feedback_fix_the_part_not_the_whole.md) — an objection to one sentence is not licence to delete the construct around it
- [Complexity may be self-imposed](~/.claude/memory/feedback_complexity_may_be_self_imposed.md) — check a constraint is real before designing around it; judge a subagent's design against what the user actually asked for
...
While working on one task the user often surfaces ideas for *other* work — "we should also debounce that search box", "remind me to cache these responses". They want these parked, not acted on, and they want Claude to hold them rather than carrying them in their own head. Memos are the backlog for exactly that: lighter than a GitHub issue (a half-formed thought, not a tracked unit of work), kept in a committed directory, `<repo>/.claude/memos/`, one markdown file per memo. Open versus addressed is which directory the file sits in — `memos/` or `memos/done/` — so nothing parses a status marker. An addressed one takes its close date onto the front of its filename and is kept for good; nothing deletes it, so there is never a clear-out to offer. The `/memo` skill owns the file format and the read/write mechanics.

One memo being one file is what lets a memo be as long as the idea needs. A title is its first line and the only part of the idea any listing shows — beside the capture stamp, and beside a `[macos]` or `[windows]` tag when the memo names one; the body below it holds the evidence, the reproduction and the reasoning, verbatim and unlimited. Write both — a title that names the thing and the change, and a body that a reader three weeks out can act on without the session it came from.

**An optional `platform:` marks the minority of memos one box cannot act on**, and marks only: nothing is hidden, renumbered or reordered by it, so a tag can never desync a listing from the number used to close a memo. Tag only when the memo's own body already says the work needs that platform — never from where it happened to be captured, which is the mistake the field makes easy. Work that is *entirely* another machine's is still a routed ping rather than a memo; the tag is for something already headed for the backlog.

**Capture — offer, don't impose, and don't act.** When the user drops an idea that isn't part of the current task, offer to memo it ("Want me to memo that?") and append it only on a yes. Never silently record, and never start on the idea — capturing it is the whole point of *not* doing it now. For deliberate captures the user runs `/memo <text>` themselves. Either way the memo lands in `.claude/memos/` in the format the `/memo` skill defines. Don't confuse a memo with an instruction: if it's plausibly "do this now", ask which they meant.

**Surface at three moments.** The backlog comes back on its own so the user never has to remember it:
- **Session start / `/clear`** — the `memos-surface.py` hook surfaces the open items in the **status bar** only (a transient "what's next" reminder), numbered newest-first to match `/memo`. It injects **nothing** into chat — so you never greet with or push the backlog — and the matching `UserPromptSubmit` hook clears the bar the moment the user sends anything. When that first message is a bare number (or "memo N" / "start N"), the hook injects, bound to that message, which memo it maps to; act on it — read the memo in full first, since the bar shows only its title, then start it as a fresh task and close it when done. A memo tagged for the other platform gets the portable part done here and the rest routed to the live session on that box, and stays open unless the portable part was all of it. Any other first message just proceeds normally, with memos unmentioned.
...
At task completion and commit, the offer is the same: present a numbered list, let the user pick one or leave them. Picking one begins a fresh task; run `memos.py done <n>` once it is genuinely done, which moves its file into `done/`.
