...
**For a `dev-server` deploy, state the URL in the report — written out in full, as `http://localhost:<DEV_PORT>`.** The script now prints it as its own last line (`open: …`), so this is a matter of carrying it through rather than deriving it. Not a port number in isolation, not implied by a config value, not buried inside `curl` output: an address someone can click. It costs one clause and it is the difference between the user looking at the thing and the user asking where it is. Then let them look — do not screenshot a page they can open themselves, at better fidelity than a screenshot, in less time than the round-trip takes.
