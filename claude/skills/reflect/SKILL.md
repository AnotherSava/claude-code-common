...
7. **Save the items** (auto-saved research findings and any approved gated items). For each:
   ...
   - Global learnings and memory are committed to a **public** repo, so evidence quoted from the user's own data — a site in their browser history, a fare, an account figure — is written generalized, keeping the shape of the measurement: "a bare host answering `301` to a page beneath it" proves as much as the real host does. `/commit` step 5 catches what slips through, but only in whichever session commits, which may not be the one that wrote it.
