...
6. **Plan your commit(s):**
   ...
   - **Implemented memos**: if a pending memo (Context **Pending memos** / `<repo>/.claude/memos/`) is clearly implemented by this change set, run `python ~/.claude/skills/memo/memos.py done <n> [<n> …]` **now**, naming every implemented memo in that one command, and fold the resulting moves into the commit that implements them. A number indexes the listing you just read and closing one renumbers the rest, so two separate `done` calls close the wrong memo; one call resolves them all first — close it *with the changes*, not as a follow-up after the push. The memo's file moves from `.claude/memos/` to `.claude/memos/done/`, so stage both paths. (Only memos this change set actually delivers; still-open ones stay untouched until step 11.)
...
10. **Hand off peer repositories:**
    ...
    - **A matching session** — draft the message, show it verbatim, and ask before sending. Approving the push is not approval to send it. Say which repo, which files, what changed and why, and that it came from a session working in this one; the receiver gets no history or files, so anything assuming shared context is unreadable there. Ask them to commit under their own review, and leave authorization out of the body entirely — routing it to that session exists so its own user decides, which a claim in the text cannot do for them. `peer_messaging.md` governs the wording; do not restate it here.
