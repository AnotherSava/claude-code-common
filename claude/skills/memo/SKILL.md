...
model: haiku
allowed-tools: Read, Edit, Bash(git rev-parse:*), Bash(date:*), Bash(tput cols:*), Bash(python3 ~/.claude/skills/memo/memos.py:*), PowerShell
---
...
- Open memos: !`python3 ~/.claude/skills/memo/memos.py count`
...
1. **Detect the terminal width** so the listing fills the screen — the helper's stdout is piped, which hides the real width, so determine it yourself:
   ...
   Subtract a 2-column gutter, then run `python3 ~/.claude/skills/memo/memos.py list --width <N>` to render the backlog (drop `--width` if you couldn't determine it — the helper falls back to ~100).
...
5. If two entries capture the same idea, do the work once and **delete** the redundant line rather than marking both `- [x]` — a duplicate checked off as done reads like two separate pieces of work in the backlog's history.
