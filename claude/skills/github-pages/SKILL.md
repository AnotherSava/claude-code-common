...
5. **Screenshots live in `docs/screenshots/`** and are referenced from every page that needs them. Keeping that set honest afterwards — dangling references, orphaned files, and shots of UI the code has since changed — belongs to `~/.claude/skills/docs-relevance/SKILL.md`, which re-checks it on every change set.
